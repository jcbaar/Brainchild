...
LRESULT OnSetParser( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	/*
	 *	Correct parameter?
	 */
	...
	if ( lpszFileName && *lpszFileName )
	{
		 ...
		 *	Setup the parser.
		 */
		return SetupParser( lpcd, lpszFileName );
	}
	return FALSE;
}
...
void FreeParserNode( POOL pMemPool, LPPARSER lpp )
{
	...
	if ( lpp->pszCommentInit  ) FreePooled( pMemPool, lpp->pszCommentInit );
	if ( lpp->pszCommentTerm  ) FreePooled( pMemPool, lpp->pszCommentTerm );
	if ( lpp->pszCommentSingle) FreePooled( pMemPool, lpp->pszCommentSingle );
...
static BOOL GetColorRef( LPTSTR lpszColorString, COLORREF *lpcrColor )
{
	 ...
	 *	Scan for the values.
	 */
	if ( _stscanf_s( lpszColorString, _T("%d,%d,%d"), &r, &g, &b ) != 3 )
		return FALSE;
...
static LPTSTR CopyArrayString( LPARRAY lpa, LPCTSTR lpszString )
{
	 ...
	 *	Allocate memory.
	 */
	size_t len = REAL_SIZE( _tcslen( lpszString ) + 1 );
	if (( lpszCopy = ArrayAllocMem( lpa, len )) != NULL )
		_tcscpy_s( lpszCopy, len, lpszString );
...
/*
 *	Parse the suppressescape command.
 */
BOOL SuppressEscape( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	 ...
	 *	Get argument.
	 */
	CHECK_BOOLEAN( bBlock.bSuppressEscape, pszArgs );
	return TRUE;
}
...
/*
 *	Parse the Marker1RGB command.
 */
BOOL Marker1RGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_1;
		lpp->crColors[ CARR_MARKER_1 ] = RGB( 128, 0, 0 );
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_1;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_1 ] );
	}
	return TRUE;
}

/*
 *	Parse the Marker2RGB command.
 */
BOOL Marker2RGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_2;
		lpp->crColors[ CARR_MARKER_2 ] = RGB( 255, 255, 0 );
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_2;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_2 ] );
	}
	return TRUE;
}

/*
 *	Parse the Marker3RGB command.
 */
BOOL Marker3RGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_3;
		lpp->crColors[ CARR_MARKER_3 ] = RGB( 255, 0, 0 );
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_3;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_3 ] );
	}
	return TRUE;
}

/*
 *	Parse the Marker4RGB command.
 */
BOOL Marker4RGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_4;
		lpp->crColors[ CARR_MARKER_4 ] = RGB( 0, 255, 0 );
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_4;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_4 ] );
	}
	return TRUE;
}

/*
 *	Parse the Marker5RGB command.
 */
BOOL Marker5RGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_5;
		lpp->crColors[ CARR_MARKER_5 ] = RGB( 0, 0, 255 );
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_5;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_5 ] );
	}
	return TRUE;
}

/*
 *	Parse the Marker1FgRGB command.
 */
BOOL Marker1FgRGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_FG_1;
		lpp->crColors[ CARR_MARKER_FG_1 ] = RGB( 255, 255, 255 );
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_FG_1;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_FG_1 ] );
	}
	return TRUE;
}

/*
 *	Parse the Marker2FgRGB command.
 */
BOOL Marker2FgRGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_FG_2;
		lpp->crColors[ CARR_MARKER_FG_2 ] = CLR_DEFAULT;
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_FG_2;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_FG_2 ] );
	}
	return TRUE;
}

/*
 *	Parse the Marker3FgRGB command.
 */
BOOL Marker3FgRGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_FG_3;
		lpp->crColors[ CARR_MARKER_FG_3 ] = CLR_DEFAULT;
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_FG_3;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_FG_3 ] );
	}
	return TRUE;
}

/*
 *	Parse the Marker4FgRGB command.
 */
BOOL Marker4FgRGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_FG_4;
		lpp->crColors[ CARR_MARKER_FG_4 ] = CLR_DEFAULT;
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_FG_4;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_FG_4 ] );
	}
	return TRUE;
}

/*
 *	Parse the Marker5FgRGB command.
 */
BOOL Marker5FgRGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_MARKER_FG_5;
		lpp->crColors[ CARR_MARKER_FG_5 ] = CLR_DEFAULT;
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_MARKER_FG_5;
		CHECK_RGB( lpp->crColors[ CARR_MARKER_FG_5 ] );
	}
	return TRUE;
}

/*
 *	Parse the ColumnOverflowRGB command.
 */
BOOL ColumnOverflowRGB( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	...
	if ( *pszArgs == _T( '*' ))
	{
		lpp->dwColorFlags |= CF_DEFAULT_COLUMN_OVERFLOW;
		lpp->crColors[ CARR_COLUMN_OVERFLOW ] = RGB( 255, 244, 247 );
	}
	else
	{
		lpp->dwColorFlags &= ~CF_DEFAULT_COLUMN_OVERFLOW;
		CHECK_RGB( lpp->crColors[ CARR_COLUMN_OVERFLOW ] );
	}
	return TRUE;
}
...
BOOL Tab( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	 ...
	 *	Get tab size.
	 */
	if ( _stscanf_s( pszArgs, _T("%ld"), &nTabSize ) != 1 )
	{
		ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
		return FALSE;
...
BOOL MaxTrack( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	 ...
	 *	Get tab size.
	 */
	if ( _stscanf_s( pszArgs, _T("%ld"), &nMaxTrack ) != 1 )
	{
		ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
		return FALSE;
...
BOOL FileMode( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	 ...
	 *	Get tab size.
	 */
	if ( _stscanf_s( pszArgs, _T("%ld"), &nFileMode ) != 1 )
	{
		ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
		return FALSE;
...
BOOL CaretType( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	 ...
	 *	Get tab size.
	 */
	if ( _stscanf_s( pszArgs, _T("%ld"), &nCaretType ) != 1 )
	{
		ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
		return FALSE;
...
BOOL UndoSize( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	 ...
	 *	Get tab size.
	 */
	if ( _stscanf_s( pszArgs, _T("%ld"), &nUndoSize ) != 1 )
	{
		ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
		return FALSE;
...
/* 
 *	Parse the LineMarginChars command. 
 */
BOOL LineMarginChars( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	int		nMarginChars;
	 ...
	 *	Get tab size.
	 */
	if ( _stscanf_s( pszArgs, _T("%ld"), &nMarginChars ) != 1 )
	{
		ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
		return FALSE;
	 ...
	 *	In range?
	 */
	if ( nMarginChars < 3 || nMarginChars > 10 )
	{
		ErrorMsg( GetString( IDS_MARGINCHARS_OUT_OF_RANGE ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
 		return FALSE;
	}

	/*
	 *	Save margin characters.
	 */
	lpp->nMarginChars = nMarginChars;

	return TRUE;
}

/* 
 *	Parse the ScrollLines command. 
 */
BOOL ScrollLines( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	int		nScrollLines;
	 ...
	 */
	if ( _stscanf_s( pszArgs, _T("%ld"), &nScrollLines ) != 1 )
	{
		ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
		return FALSE;
	 ...
	 *	In range?
	 */
	if ( nScrollLines < 1 || nScrollLines > 10 )
	{
		ErrorMsg( GetString( IDS_SCROLLLINES_OUT_OF_RANGE ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
 		return FALSE;
	}
...
BOOL Hardcoded( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	...
	if (( lpCNode = AllocPooled( pMemPool, sizeof( CNODE ))) != NULL )
	{
		 ...
		 */
		if ( _stscanf_s( pszArgs, _T("%d"), &nCode ) == 1 )
		{
			/*
			 *	Command valid?
			 */
...
/*
 *	Parse the savetabsasspaces command.
 */
BOOL SaveTabsAsSpaces( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	 ...
	 *	Get argument.
	 */
	CHECK_BOOLEAN( lpp->bSaveTabsAsSpaces, pszArgs );
	return TRUE;
}
...
BOOL BackupMode( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	 ...
	 *	Get backup mode.
	 */
	if ( _stscanf_s( pszArgs, _T("%ld"), &nBackupMode ) != 1 )
	{
		ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
		return FALSE;
...
BOOL Font( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs, BOOL bPrinter )
{
	...
	if ( pszArgs[ i ] == _T( ',' ))
	{
		 ...
		 */
		if ( _stscanf_s( &pszArgs[ i ], _T("%ld,%ld,%ld,%ld,%ld"), &lf.lfHeight, &lf.lfWeight, &nCharSet, &nItalic, &nUnderline ) != 5 )
		{
			ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
			bErrorMsg = TRUE;
			return FALSE;
	...
	/*
	 *	Create new font (if it is a screen font).
	 */
	if ( bPrinter == FALSE )
	{
	...
	{
		/*
		 *	Copy the font information. When printing
		 *	the actual font is created.
		 */
...
/*
 *	Parser the comment commands.
 */
BOOL CommentInit( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	return ReadString( pMemPool, lpp, nLineNum, lpszFileName, pszArgs, &lpp->pszCommentInit );
}

BOOL CommentTerm( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	return ReadString( pMemPool, lpp, nLineNum, lpszFileName, pszArgs, &lpp->pszCommentTerm );
}

BOOL CommentSingle( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	return ReadString( pMemPool, lpp, nLineNum, lpszFileName, pszArgs, &lpp->pszCommentSingle );
}
...
/*
 *	Parse the colorcolumnoverflow command.
 */
BOOL ColorColOverflow( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	/*
	 *	Check for arguments.
	 */
	 ...
	 *	Get argument.
	 */
	CHECK_BOOLEAN( lpp->bColorColumnMarkerOverflow, pszArgs );
	return TRUE;
}
...
BOOL ColMarkerOffset( POOL pMemPool, LPPARSER lpp, int nLineNum, LPCTSTR lpszFileName, TCHAR *pszArgs )
{
	 ...
	 *	Get tab offset.
	 */
	if ( _stscanf_s( pszArgs, _T("%ld"), &nColOffset ) != 1 )
	{
		ErrorMsg( GetString( IDS_PARSING_ERROR ), nLineNum, lpszFileName );
		bErrorMsg = TRUE;
		return FALSE;
...
COMM CommandTable[] = {
	...
	{ _T("End="),			4,		End			},
	{ _T("SuppressEscape="),	15,		SuppressEscape		},
	...
	{ _T("SaveTabsAsSpaces="),	17,		SaveTabsAsSpaces	},
	...
	{ _T("Marker1RGB="),		11,		Marker1RGB		},
	{ _T("Marker2RGB="),		11,		Marker2RGB		},
	{ _T("Marker3RGB="),		11,		Marker3RGB		},
	{ _T("Marker4RGB="),		11,		Marker4RGB		},
	{ _T("Marker5RGB="),		11,		Marker5RGB		},
	{ _T("Marker1FgRGB="),		13,		Marker1FgRGB		},
	{ _T("Marker2FgRGB="),		13,		Marker2FgRGB		},
	{ _T("Marker3FgRGB="),		13,		Marker3FgRGB		},
	{ _T("Marker4FgRGB="),		13,		Marker4FgRGB		},
	{ _T("Marker5FgRGB="),		13,		Marker5FgRGB		},
	{ _T("ColumnOverflowRGB="),	18,		ColumnOverflowRGB	},
	...
	{ _T("LineMarginChars="),	16,		LineMarginChars		},
	{ _T("CommentInit="),		12,		CommentInit		},
	{ _T("CommentTerm="),		12,		CommentTerm		},
	{ _T("CommentSingle="),		14,		CommentSingle		},
	{ _T("ScrollLines="),		12,		ScrollLines		},
	...
	{ _T("ColorColumnOverflow="),	20,		ColorColOverflow	},
...
void DefaultParser( LPPARSER lpp )
{
	...
	lpp->bSaveTabsAsSpaces	  = FALSE;
	lpp->bColorColumnMarkerOverflow = FALSE;
	...
	/*
	 *	No commenting stuff by default.
	 */
	lpp->pszCommentInit	= NULL;
	lpp->pszCommentTerm	= NULL;
	lpp->pszCommentSingle	= NULL;
	...
	_tcscpy_s( lpp->lfScreenFont.lfFaceName, LF_FACESIZE, _T("Courier New"));
	...
	_tcscpy_s( lpp->lfScreenFontUL.lfFaceName, LF_FACESIZE, ("Courier New"));
...
BOOL UnloadParser( LPCTSTR pszTitle )
{
	/*
	 *	Lookup the parser.
	 */
	...
	if ( LockParserList())
	{
		...
		{
			/*
			 *	Is this the one we are looking for and was it loaded
			 *	by one of the LoadParserFromXXX() functions?
			 */
			if ( _tcsicmp( lpp->pszTitle, pszTitle ) == 0 && lpp->bHasBeenAdded )
			{
				/*
				 *	Yes. Remove it from the list and update the
				 *	settings.
				 */
				Remove(( LPNODE )lpp );
				UnlockParserList();
...
BOOL LoadParserFile( POOL pMemPool, LPPARSERLIST lppl, LPCTSTR lpszFileName, HGLOBAL hGlobal, BOOL bAdded )
{
	...
	while ( 1 )
	{
		...
		if ( pszResData == NULL )
		{
			if ( _tfopen_s( &file, lpszFileName, _T( "r" )) != 0 )
			{
				bError = TRUE;
				break;
			}
