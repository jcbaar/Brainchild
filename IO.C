...
void SetFileName( LPCLASSDATA lpcd, LPCTSTR lpszFileName )
{
	 ...
	 *	Copy the file name.
	 */
	_tcscpy_s( &lpcd->szFileName[ 0 ], MAX_PATH, lpszFileName );
...
LRESULT OnLoadFile( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	 ...
	 */
	if ( lParam != 0L )
		_tcscpy_s( szFileName, MAX_PATH, ( LPCTSTR )lParam );
	else
		/*
...
static BOOL FileBackup( LPCLASSDATA lpcd )
{
	...
	if ( Parser->bBackup )
	{
		 ...
		 *	the buffer.
		 */
		_tcscpy_s( szPath, MAX_PATH, lpcd->szFileName );
		...
		switch ( Parser->nBackupMode )
		{
			case	BMODE_FILE_BAK:
				 ...
				 */
				if (( pszPtr = _tcsrchr( szPath, _T( '.' ))) != NULL ) _tcscpy_s( pszPtr, MAX_PATH, pszBak );
				else						      _tcscat_s( szPath, MAX_PATH, pszBak );
				break;
			...
			case	BMODE_FILE_BAK_EXT:
				...
				{
					_tcscpy_s( szExt, MAX_PATH, pszPtr );
					_tcscpy_s( pszPtr, MAX_PATH - ( pszPtr - szPath ), pszBak );
					_tcscat_s( szPath, MAX_PATH, szExt );
				}
				else
					_tcscat_s( szPath, MAX_PATH, pszBak );
				break;
			...
			case	BMODE_FILE_EXT_PATH:
				...
				{
					_tcscpy_s( szPath, MAX_PATH, Parser->pszBackupPath );
				 ...
				 *	Append file name.
				 */
				_tcscat_s( szPath, MAX_PATH, pszPtr );
				break;
		}
...
LRESULT OnSaveFile( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	 ...
	 *	File name passed?
	 */
	if ( lParam != 0L ) _tcscpy_s( szFileName, MAX_PATH, ( LPCTSTR )lParam );
	else		    _tcscpy_s( szFileName, MAX_PATH, lpcd->szFileName );
