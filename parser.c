...
static BOOL BlockEnd( LPCLASSDATA lpcd, LPBLOCK lpb, TCHAR *pcText, int nIndex, int nLength )
{
	...
	if ( lpb->nEnd || lpb->pszEnd == END_OF_LINE )
	{
		...
		if ( lpb->pszEnd == END_OF_LINE )
		{
			...
			{
				/*
				 *	Do we check for escape characters?
				 */
				if ( lpb->bSuppressEscape == FALSE )
				{
					/*
					...
					{
						/*
						 *	Count the number of continues escape characters 
						 *	located directly before the eol.
						 */
						int nPos = nIndex - 1, nNum = 0;
						while ( pcText[ nPos ] == Parser->cEscape && nPos )
						{
							nNum++;
							nPos--;
						}
						...
						/*
 						 *	If there were an even number of escape characters
						 *	it means that the block will be terminated.
						 */
						if (( nNum & 1 ) == 0 )
							/*
							 *	Still a match.
		...
		else if ( ! ( Parser->bCaseOn ? _tcsncmp : _tcsnicmp )( lpb->pszEnd, &pcText[ nIndex ], lpb->nEnd ))
		{
			...
			if ( lpb->bSuppressEscape == FALSE )
			{
				...
				if ( nIndex > 0 && Parser->cEscape && pcText[ nIndex - 1 ] == Parser->cEscape )
				{
					/*
					 *	Count the number of continues escape characters 
					 *	located directly before the terminator.
					 */
					int nPos = nIndex - 1, nNum = 0;
					while ( pcText[ nPos ] == Parser->cEscape && nPos )
					{
					...
					/*
					 *	An uneven number of escape characters means that the
					 *	block terminator itself is escaped.
					 */
					return FALSE;
				}
			}
...
/*
 *	Defined in hyperlink.c
 */
extern HLPREFIX Hyper[];
