...
LRESULT OnSetColor( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	static DWORD dwFlags[] =
	{
		...
		CF_DEFAULT_MARKER_4,
		CF_DEFAULT_MARKER_5,
		CF_DEFAULT_MARKER_FG_1,
		CF_DEFAULT_MARKER_FG_2,
		CF_DEFAULT_MARKER_FG_3,
		CF_DEFAULT_MARKER_FG_4,
		CF_DEFAULT_MARKER_FG_5,
		CF_DEFAULT_COLUMN_OVERFLOW
	};
...
LRESULT OnSysColorChange( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	...
	if (( hBrush = CreateSolidBrush( lpp->crColors[ CARR_COLUMN_OVERFLOW ] )) != NULL )
	{
		DeleteObject( lpcd->hbColumnOverflow );
		lpcd->hbColumnOverflow = hBrush;
	}

	for ( i = 0; i < MAXMARKERS - 1; i++ )
	{
		if (( hBrush = CreateSolidBrush( lpp->crColors[ CARR_MARKER_1 + i ] )) != NULL )
		{
			DeleteObject( lpcd->hbMarkers[ i ] );
			lpcd->hbMarkers[ i ] = hBrush;
		}
	}
...
LRESULT OnCreate( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpDummy )
{
	...
	if ( lpcd != NULL )
	{
		...
		if (( lpcd->hHand = LoadCursor( hDLL, MAKEINTRESOURCE( IDC_HYPERLINK ))) == NULL )
			return -1;
		if (( lpcd->hIncremental = LoadCursor( hDLL, MAKEINTRESOURCE( IDC_INCREMENTAL ))) == NULL )
			return -1;
		...
		lpcd->hbMarkers[ 0 ]	     = CreateSolidBrush( lpp->crColors[ CARR_MARKER_1 ] );
		lpcd->hbMarkers[ 1 ]	     = CreateSolidBrush( lpp->crColors[ CARR_MARKER_2 ] );
		lpcd->hbMarkers[ 2 ]	     = CreateSolidBrush( lpp->crColors[ CARR_MARKER_3 ] );
		lpcd->hbMarkers[ 3 ]	     = CreateSolidBrush( lpp->crColors[ CARR_MARKER_4 ] );
		lpcd->hbMarkers[ 4 ]	     = CreateSolidBrush( lpp->crColors[ CARR_MARKER_5 ] );
		lpcd->hbColumnOverflow	     = CreateSolidBrush( lpp->crColors[ CARR_COLUMN_OVERFLOW ] );
		 ...
		 *	OK?
		 */
		if ( lpcd->hbBackground && lpcd->hbBackgroundRO && lpcd->hbSelectionMargin && lpcd->hbSolidSelectionMargin && 
		     lpcd->hbBookmark && lpcd->hPattern && lpcd->hbSelBkgnd && lpcd->hbLineMargin && lpcd->hPatternLine && 
		     lpcd->hbLineMarginBkgnd && lpcd->hbLineHighlight && lpcd->hbMarkers[ 0 ] && lpcd->hbMarkers[ 1 ] &&
		     lpcd->hbMarkers[ 2 ] && lpcd->hbMarkers[ 3 ] && lpcd->hbMarkers[ 4 ] )
		{
			/*
			...
			/*
			 *	Setup default marker definitions.
			 */
...
LRESULT OnDestroy( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	...
	if ( lpcd )
	{
		...
		/*
		 *	Free the Incremental search string.
		 */
		if ( lpcd->lpszIncrementalSearch ) FreePooled( lpcd->pMemPool, ( LPVOID )lpcd->lpszIncrementalSearch );
		...
		if ( lpcd->hIncremental ) DestroyCursor( lpcd->hIncremental );
		...
		if ( lpcd->hbColumnOverflow	  ) DeleteObject( lpcd->hbColumnOverflow );

		for ( i = 0; i < MAXMARKERS - 1; i++ )
		{
			if ( lpcd->hbMarkers[ i ] ) DeleteObject( lpcd->hbMarkers[ i ] );
		}
