...
static MoveCaret( LPCLASSDATA lpcd )
{
	 ...
	 */
	SetCaretPos( GetMarginWidth( lpcd ) + (( GetCaretOffset( lpcd, lpcd->ptCaretPos.x ) - lpcd->ptViewPos.x ) * Parser->szCharSize.cx ), ( lpcd->ptCaretPos.y - lpcd->ptViewPos.y ) * Parser->szCharSize.cy );
}
...
static LRESULT STDMETHODCALLTYPE IDropTarget_DragEnter( LPDROPTARGET lpdt, LPDATAOBJECT lpdo, DWORD dwKeyState, POINTL pt, DWORD *lpdwEffect )
{
	...
	if ( ValidData( lpdo ))
	{
		 ...
		 */
		point.x = pt.x - GetMarginWidth( lpcd );
		point.y = pt.y;
...
static void MoveView( LPCLASSDATA lpcd )
{
	 ...
	 *	client area?
	 */
	if ( ptMouse.x < ( GetMarginWidth( lpcd ) + ( Parser->szCharSize.cx / 2 ))) ptView.x--;
