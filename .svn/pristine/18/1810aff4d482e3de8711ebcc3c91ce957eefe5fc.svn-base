...
LRESULT OnSetFocus( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	...
	/*
	 *	Hide all search and replace dialogs that are not
	 *	owned by this window.
	 */
	HideSearchDialogs( lpcd->hWnd );

	/*
	 *	Any Qualifier keys pressed?
	 */
	if ( GetAsyncKeyState( VK_CONTROL ) & ( 1 << 15 )) lpcd->cQualifier |= QUAL_CTRL;
	if ( GetAsyncKeyState( VK_SHIFT   ) & ( 1 << 15 )) lpcd->cQualifier |= QUAL_SHIFT;
	if ( GetAsyncKeyState( VK_MENU    ) & ( 1 << 15 )) lpcd->cQualifier |= QUAL_ALT;
	return 0;
}
...
void DisplayCaret( LPCLASSDATA lpcd, BOOL bShow )
{
	...
	if ( bShow != lpcd->bCaretVisible )
	{
		...
		if (( lpcd->bCaretVisible = bShow ) == TRUE )
		{
			...
			SetCaretPos( GetMarginWidth( lpcd ) + (( GetCaretOffset( lpcd, lpcd->ptCaretPos.x ) - lpcd->ptViewPos.x ) * Parser->szCharSize.cx ), ( lpcd->ptCaretPos.y - lpcd->ptViewPos.y ) * Parser->szCharSize.cy + cy );
			ShowCaret( lpcd->hWnd );
		}
		else
			HideCaret( lpcd->hWnd );
...
void CaretUp( LPCLASSDATA lpcd )
{
	...
	{
		/*
		 *	Note the start marker position.
		 */
		POINT ptPos = lpcd->ptSelStart;
		...
		/*
		 *	Move to the start marker position.
		 */
		lpcd->ptCaretPos = ptPos;
	}
...
void CaretDown( LPCLASSDATA lpcd )
{
	...
	{
		/*
		 *	Note the end marker position.
		 */
		POINT ptPos = lpcd->ptSelEnd;
		...
		/*
		 *	Move to the end marker position.
		 */
		lpcd->ptCaretPos = ptPos;
	}
