...
LRESULT OnNumCharsInLine( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	LPLINE		lpLine;
	...
	{
		/*
		*	Return the length with tabs converted to spaces?
		*/
		if ( lParam )
			/*
			*	Return the length of the line as if the tabs
			*	were spaces.
			*/
			return CaretOffsetLine( lpcd, wParam == -1 ? lpcd->ptCaretPos.y : wParam, lpLine->nLength );
		else
			/*
			*	Return the length of the line.
			*/
			return lpLine->nLength;
	}
	/*
	 *	Non-existing line.
	 */
...
LRESULT OnSetText( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	 ...
	 *	Get the file name.
	 */
	_tcscpy_s( szName, MAX_PATH, GetString( IDS_NO_NAME ));
	...
	if (( pszTmp = _tcsrchr( lpcd->szFileName, _T( '.' ))) != NULL )
		_tcscat_s( szName, MAX_PATH, pszTmp );
	...
	{
		if ( InsertLine( lpcd, NULL, 0, -1, FALSE ) == FALSE )
			return FALSE;
	}
...
LRESULT OnInsertLine( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	 ...
	 */
	if ( InsertLine( lpcd, ( LPCTSTR )lParam, -1, nPos, TRUE ) == FALSE )
	{
		DisplayCaret( lpcd, TRUE );
		return FALSE;
	}
...
void DeleteContents( LPCLASSDATA lpcd )
{
	...
	{
		lpcd->aMarkers[ i ].nCurrentLine = -1;
		lpcd->aMarkers[ i ].nAnchor = -1;
		lpcd->aMarkers[ i ].nMarkers = 0;
	}
...
BOOL InsertLine( LPCLASSDATA lpcd, LPCTSTR lpszText, int nLength, int nPosition, BOOL bSetState )
{
	LINE		line = { 0 };
	BOOL		bAdd;
			...
			switch ( c )
			{
				case	_T( '\r' ):
					...
					if ( Parser->nFileMode == FMODE_MACINTOSH || Parser->nFileMode == FMODE_MSDOS )
					{
						 ...
						 *	Add the line.
						 */
						if ( InsertLine( lpcd, pcLineBuf, nLineLen, -1, FALSE ) ==  FALSE )
						{
							bError = TRUE;
							break;
						}
...
static int ConvertTabsToSpaces( LPCLASSDATA lpcd, TCHAR *pcInput, TCHAR *pcOutput, int nLength )
{
	int	 i, nPos = 0, nNewLen = 0;
...
{
	HANDLE		hFile = INVALID_HANDLE_VALUE;
	int		nLines, nCrSize, i, nLength;
	DWORD		dwBytesWritten, dwFileAttr = lpcd->dwFileAttr;
	BOOL		bError = FALSE;
	TCHAR	       *pcBuffer = NULL, *pcOutput;

	/*
	 *	Do we convert tabs to spaces?
	 */
	if ( Parser->bSaveTabsAsSpaces )
	{
		/*
		 *	Allocate a buffer to hold the converted text.
		 */
	...
	while ( 1 )
	{
		...
		for ( i = 0; i < nLines; i++ )
		{
			...
			if ( lpLine->pcText )
			{
				...
				if ( Parser->bSaveTabsAsSpaces )
				{
					nLength = ConvertTabsToSpaces( lpcd, lpLine->pcText, pcBuffer, lpLine->nLength );
					pcOutput = pcBuffer;
				}
				else
				{
					pcOutput = lpLine->pcText;
					nLength  = lpLine->nLength;
				}

				if ( ! ( lpcd->bUnicode ? WriteUnicodeFile : WriteFile )( hFile, pcOutput, nLength, &dwBytesWritten, NULL ))
				{
					bError = TRUE;
					break;
				}
				if (( int )dwBytesWritten != nLength )
				{
					bError = TRUE;
					break;
				}
...
static BOOL StaticInsertText( LPCLASSDATA lpcd, int nLine, int nColumn, LPCTSTR lpszText, LPPOINT ptNewPos )
{
	...
	while ( 1 )
	{
		...
		else
		{
			 ...
			 *	Insert a new line.
			 */
			bError = ! InsertLine( lpcd, lpszText, nIndex, nCurrent, TRUE );
		}
...
static BOOL StaticDeleteText( LPCLASSDATA lpcd, LPPOINT ptFrom, LPPOINT ptTo )
{
	 ...
	 */
	if ( ArrayGetSize( lpcd->lpLines ) == 0 )
		InsertLine( lpcd, NULL, 0, -1, FALSE );
...
static void RemoveLastUndoRecord( LPCLASSDATA lpcd )
{
	...
	if ( lpcd->nUndoPosition )
	{
		 ...
		 *	Free it's text.
		 */
		if ( lpUndoRedo->uUndoInfo.sUndo.pcText )
			ArrayFreeMem( lpcd->lpUndoRedo, lpUndoRedo->uUndoInfo.sUndo.pcText );
...
{
	LPUNDOREDO	lpRecord, lpUndoRedo, lpNext;
	int		nCount, i;
	BOOL		bRC = TRUE;
	BOOL		bSingleGroup = TRUE;
	...
	{
		/*
		 *	Free any text/data which might be attached to the records.
		 */
		for ( i = lpcd->nUndoPosition + 1; i < nCount; i++ )
		{
			/*
			 *	Get the record.
			 */
			lpRecord = ArrayGetAt( lpcd->lpUndoRedo, i );

			/*
			 *	Is it a linesort record?
			 */
			if (( lpRecord->dwFlags & URF_LINESORT ) == URF_LINESORT )
			{
				/*
				 *	Free the data.
				 */
				if ( lpRecord->uUndoInfo.sSort.lpSortLines )
					ArrayFreeMem( lpcd->lpUndoRedo, lpRecord->uUndoInfo.sSort.lpSortLines );
			}
			else if ( lpRecord->uUndoInfo.sUndo.pcText ) 
				/*
				 *	Free the text.
				 */
				ArrayFreeMem( lpcd->lpUndoRedo, lpRecord->uUndoInfo.sUndo.pcText );
		}
	...
	if ( ArrayGetSize( lpcd->lpUndoRedo ) == lpcd->nUndoSize )
	{
		...
		/*
		 *	Check to see if the undo buffer
		 *	contains more than a single group.
		 */
		for (;;)
		{
			/*
			 *	Did we do then all?
			 */
			if ( nCount == lpcd->nUndoSize )
				break;

			/*
			 *	Get this record and check if it is the start
			 *	of a new group.
			 */
			lpUndoRedo = ArrayGetAt( lpcd->lpUndoRedo, nCount++ );
			if (( lpUndoRedo->dwFlags & URF_GROUP ) == URF_GROUP )
			{
				/*
				 *	If this is the second group start we counted
				 *	in the undo buffer then we can stop here. The
				 *	undo buffer does not exist of a single group.
				 */
				if ( nGrpCnt++ > 1 )
				{
					bSingleGroup = FALSE;
		...
		/*
		 *	Does the complete buffer contain only a single group?
		 */
		if ( bSingleGroup )
		{
			/*
			 *	Yes. This means that we start removing a single
			 *	record from the start of the buffer so we make
			 *	room for a single new record at the end of the
			 *	buffer.
			 *
			 *	This also means that when undoing the change only
			 *	lpcd->nUndoSize records of the large single group
			 *	will be undone.
			 */
			nCount = 1;
		}
		...
		{
			/*
			 *	Remove the first group completely from the
			 *	undo buffer.
			 */
			nCount = 0;
			for (;;)
			{
				 ...
				 */
				lpUndoRedo = ArrayGetAt( lpcd->lpUndoRedo, nCount++ );
				lpNext = ArrayGetAt( lpcd->lpUndoRedo, nCount );

				/*
				 *	Is it a linesort record?
				 */
				if (( lpUndoRedo->dwFlags & URF_LINESORT ) == URF_LINESORT )
				{
					/*
					 *	Free the data.
					 */
					if ( lpUndoRedo->uUndoInfo.sSort.lpSortLines )
						ArrayFreeMem( lpcd->lpUndoRedo, lpUndoRedo->uUndoInfo.sSort.lpSortLines );
				}
				/*
				 *	Delete text.
				 */
				else if ( lpUndoRedo->uUndoInfo.sUndo.pcText )
					/*
					 *	Free the text.
					 */
					ArrayFreeMem( lpcd->lpUndoRedo, lpUndoRedo->uUndoInfo.sUndo.pcText );
	...
	if (( lpRecord = ArrayAllocMem( lpcd->lpUndoRedo, sizeof( UNDOREDO ))) != NULL )
	{
		 ...
		 *	Initalize it.
		 */
		lpRecord->uUndoInfo.sUndo.ptStartPos = *ptStart;
		lpRecord->uUndoInfo.sUndo.ptEndPos   = *ptEnd;
		...
		if ( HasMark( lpcd ))
		{
			lpRecord->ptStartSel = lpcd->ptSelStart;
			lpRecord->ptEndSel   = lpcd->ptSelEnd;
			lpRecord->dwFlags   |= URF_SELECTION;
		}

		/*
		 *	Is this a linesort record?
		 */
		if ( lpRecord->dwFlags & URF_LINESORT )
		{
			/*
			 *	The lpszText parameter actually points to the
			 *	LPARRAY of sorted lines.
			 */
			LPARRAY	   pArray = ( LPARRAY )lpszText;
			LPSORTLINE pLines;

			/*
			 *	We need to copy the lines to the Undo/Redo
			 *	array memory pool.
			 */
			if (( pLines =  ArrayAllocMem( lpcd->lpUndoRedo, ArrayGetSize( pArray ) * sizeof( SORTLINE ))) == NULL )
				return FALSE;

			/*
			 *	Loop through the sortline array to copy
			 *	the data in the allocated memory.
			 */
			for ( i = 0; i < ArrayGetSize( pArray ); i++ )
			{
				/*
				 *	Copy the data into the allocated
				 *	memory.
				 */
				pLines[ i ] = *( LPSORTLINE )ArrayGetAt( pArray, i );
			}

			/*
			 *	Setup the information in the record.
			 */
			lpRecord->uUndoInfo.sSort.lpSortLines = pLines;
			lpRecord->uUndoInfo.sSort.ptLines     = *ptStart;
		}
		/*
		 *	Any text?
		 */
		else if ( lpszText && *lpszText )
		{
			 ...
			 *	Allocate buffer.
			 */
			size_t len = REAL_SIZE( _tcslen( lpszText ) + 1 );
			if (( lpRecord->uUndoInfo.sUndo.pcText = ArrayAllocMem( lpcd->lpUndoRedo, len )) != NULL )
				/*
				 *	Copy the input text.
				 */
				_tcscpy_s( lpRecord->uUndoInfo.sUndo.pcText, len, lpszText );
			else
			{
				/*
				 *	Free record.
	...
	if ( lpcd->nUndoPosition > 0 ) 
	{
		...
		while ( 1 )
		{
			...
			if (( lpUndoRedo->dwFlags & URF_INSERT ) == URF_INSERT )
			{
				 ...
				 */
				StaticDeleteText( lpcd, &lpUndoRedo->uUndoInfo.sUndo.ptStartPos, &lpUndoRedo->uUndoInfo.sUndo.ptEndPos );

				/*
				 *	Caret at the start position.
				 */
				lpcd->ptCaretPos = lpUndoRedo->uUndoInfo.sUndo.ptStartPos;

			}
			else if ((( lpUndoRedo->dwFlags & URF_DELETE ) == URF_DELETE ) && lpUndoRedo->uUndoInfo.sUndo.pcText )
			{
				/*
				 *	Insert the deleted text
				 */
				StaticInsertText( lpcd, lpUndoRedo->uUndoInfo.sUndo.ptStartPos.y, lpUndoRedo->uUndoInfo.sUndo.ptStartPos.x, lpUndoRedo->uUndoInfo.sUndo.pcText, NULL );

				/*
				 *	Caret at the end position.
				 */
				lpcd->ptCaretPos = lpUndoRedo->uUndoInfo.sUndo.ptEndPos;
			}
			else if ((( lpUndoRedo->dwFlags & URF_LINESORT ) == URF_LINESORT ) && lpUndoRedo->uUndoInfo.sSort.lpSortLines )
			{
				/*
				 *	Unsort the lines.
				 */
				InternalUnsortLines( lpcd, lpUndoRedo->uUndoInfo.sSort.lpSortLines, lpUndoRedo->uUndoInfo.sSort.ptLines );
			...
			{
				/*
				 *	Get a pointer to the lines to swap.
				 */
				LPLINE lpA = ( LPLINE )ArrayGetAt( lpcd->lpLines, lpUndoRedo->uUndoInfo.sUndo.ptStartPos.x );
				LPLINE lpB = ( LPLINE )ArrayGetAt( lpcd->lpLines, lpUndoRedo->uUndoInfo.sUndo.ptStartPos.y );
			...
			{
				/*
				 *	Does it contain a selection marker?
				 */
				if ( lpUndoRedo->dwFlags & URF_SELECTION )
				{
					/*
					 *	Yes. Restore the selection.
					 */
					lpcd->ptSelStart = lpUndoRedo->ptStartSel;
					lpcd->ptSelEnd   = lpUndoRedo->ptEndSel;
					
					/*
					 *	Update dialog if present.
					 */
					if ( lpcd->hReplaceWnd )
						SendMessage( lpcd->hReplaceWnd, WMF_CHECKSELECTION, 0, 0 );
				}
				else  if ( ! lpcd->bSilentMode && HasMark( lpcd ))
				{
					ClearMark( lpcd );
				}
				break;
		...
		if ( ! lpcd->bSilentMode )
		{
			 ...
			 */
			if ( ! bRedraw && nNumRecords == 1 && lpUndoRedo->uUndoInfo.sUndo.ptStartPos.y == lpUndoRedo->uUndoInfo.sUndo.ptEndPos.y )
			{
				/*
				 *	Re-render a single line.
				 */
				 RenderLine( lpcd, lpUndoRedo->uUndoInfo.sUndo.ptStartPos.y );
				 SetupHScroller( lpcd );
			}
			else
			{
...
void Redo( LPCLASSDATA lpcd )
{
	...
	if ( lpcd->nUndoPosition < nNumElem )
	{
		...
		while ( 1 )
		{
			 ...
			 *	Redo text insert?
			 */
			if ((( lpUndoRedo->dwFlags & URF_INSERT ) == URF_INSERT ) && lpUndoRedo->uUndoInfo.sUndo.pcText )
			{
				/*
				 *	Re-insert the text.
				 */
				StaticInsertText( lpcd, lpUndoRedo->uUndoInfo.sUndo.ptStartPos.y, lpUndoRedo->uUndoInfo.sUndo.ptStartPos.x, lpUndoRedo->uUndoInfo.sUndo.pcText, &lpcd->ptCaretPos );
			...
			else  if (( lpUndoRedo->dwFlags & URF_DELETE ) == URF_DELETE )
			{
				/*
				 *	Re-delete the text.
				 */
			...
			else if ((( lpUndoRedo->dwFlags & URF_LINESORT ) == URF_LINESORT ) && lpUndoRedo->uUndoInfo.sSort.lpSortLines )
			{
				 ...
				 *	Re-sort the lines.
				 */
				InternalSortLines( lpcd, &lpUndoRedo->uUndoInfo.sSort.ptLines, TRUE );

				/*
				 *	Force a complete redrwa.
				 */
				bRedraw = TRUE;
			}
			else
			{
				/*
				 *	Get a pointer to the lines to swap back.
				 */
				LPLINE lpA = ( LPLINE )ArrayGetAt( lpcd->lpLines, lpUndoRedo->uUndoInfo.sUndo.ptStartPos.x );
				LPLINE lpB = ( LPLINE )ArrayGetAt( lpcd->lpLines, lpUndoRedo->uUndoInfo.sUndo.ptStartPos.y );
			...
			if ((( lpNext->dwFlags & URF_GROUP ) == URF_GROUP ) || lpcd->nUndoPosition == ArrayGetSize( lpcd->lpUndoRedo ))
			{
				 ...
				 *	Does it contain a selection marker?
				 */
				if ( lpNext->dwFlags & URF_SELECTION )
				{
					/*
					 *	Yes. Restore the selection.
					 */
					lpcd->ptSelStart = lpNext->ptStartSel;
					lpcd->ptSelEnd   = lpNext->ptEndSel;
		...
		if ( ! lpcd->bSilentMode )
		{
			...
			/*
			 *	Was the undo on a single line and do we
			 *	not have a marker set?
			 */
			if ( ! bRedraw && ! HasMark( lpcd ) && nNumRecords == 1 && lpUndoRedo->uUndoInfo.sUndo.ptStartPos.y == lpUndoRedo->uUndoInfo.sUndo.ptEndPos.y )
			{
				/*
				 *	Re-render a single line.
				 */
