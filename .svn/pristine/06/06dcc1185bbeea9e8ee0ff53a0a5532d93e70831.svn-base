...
void VScroll( LPCLASSDATA lpcd, int nLines )
{
	...
	if ( abs( nLines ) < lpcd->szViewSize.cy )
	{
		 ...
		 */
		/*rcView.right  = rcView.left + ( lpcd->szViewSize.cx  * Parser->szCharSize.cx ) + GetMarginWidth( lpcd ); */
...
void HScroll( LPCLASSDATA lpcd, int nCols )
{
	...
	if ( abs( nCols ) < lpcd->szViewSize.cx )
	{
		 ...
		 *	Adjust for a view size.
		 */
		rcView.left  += GetMarginWidth( lpcd );
		...
		/*
		 *	Re-render all the visible line which have
		 *	markers set.
		 */
		for ( nLine = 0; nLine < lpcd->szViewSize.cy; nLine++ )
		{
			pLine = ( LPLINE )ArrayGetAt( lpcd->lpLines, nLine + lpcd->ptViewPos.y );
			if ( pLine && pLine->dwFlags & LNF_MARKER )
				RenderLine( lpcd, nLine + lpcd->ptViewPos.y );
		}
	}
	else
		/*
...
int GetSelMarginWidth( LPCLASSDATA lpcd )
{
	int	nMargin = 0;
...
int GetLineMarginWidth( LPCLASSDATA lpcd )
{
	...
	if ( Parser->bLineNumbers )
	{
		...
		/*
		 *	The line number margin is 'MarginChars' times the character
		 *	width.
		 */
		nMargin = ( Parser->szCharSize.cx * Parser->nMarginChars ) + 6;
	}
...
{
	/*
	 *	Return the total margin width.
	 */
	return GetSelMarginWidth( lpcd ) + GetLineMarginWidth( lpcd ) + 5;
}
...
void SetupViewSize( LPCLASSDATA lpcd )
{
	 ...
	 *	Compute view size.
	 */
	lpcd->szViewSize.cx = ( RCWIDTH( rcClient ) - GetMarginWidth( lpcd )) / Parser->szCharSize.cx;
...
static void ShowSelection( LPCLASSDATA lpcd, HDC hDC, int nLine, int nLength, TCHAR *pcBuf )
{
	...
	if ( nLine >= lpcd->ptSelStart.y && nLine <= lpcd->ptSelEnd.y )
	{
		 ...
		 *	Adjust for the view...
		 */
		rcRect.left  += GetMarginWidth( lpcd );
		...
		if (( lpLine = ArrayGetAt( lpcd->lpLines, nLine )) != NULL )
		{
			...
			if (( pcBuf = lpcd->pszBuffer ) != NULL )
			{
...

				{
					...
					SYNTAX_COLOR	 scMarker = { CLR_DEFAULT, CLR_DEFAULT, nNewLen, NULL };
					BOOL		 bBreak = FALSE;
					int		 nBlocks = 1;
...

					{
						 ...
						 *	rendering offsets.
						 */
						int	 cx = GetMarginWidth( lpcd ), cy = ( nLine - lpcd->ptViewPos.y ) * Parser->szCharSize.cy;
						int	 nWillFit;
						int	 nOldMode = GetBkMode( hDC );
						int	 nMarkerOffset = 0, i;
						COLORREF crOldFg = GetTextColor( hDC );
						COLORREF crOldBg = GetBkColor( hDC );

						/*
						 *	Fix casing. We do not do case-fixing if this line is completely inside
						 *	a text selection.
						 */
						if ( ! HasMark( lpcd ) || nLine <= lpcd->ptSelStart.y || nLine >= lpcd->ptSelEnd.y )
							FixCasing( lpcd, lpLine, nLine, pcBuf, scArray, nBlocks );

						/*
						 *	Is there a line marker set on this line??
						 */
						if ( lpLine->dwFlags & LNF_MARKER )
						{
							/*
							 *	Iterate the markers.
							 */
							RECT	rcLine;
							HBRUSH	hBrush;
							for ( i = 1; i < MAXMARKERS; i++ )
							{
								/*
								 *	Is it this one?
								 */
								if ( lpLine->dwFlags & ( 1 << i ))
								{
									/*
									 *	Get the brush for this marker.
									 */
									hBrush = lpcd->hbMarkers[ i - 1 ];

									/*
									 *	Setup the marker text color.
									 */
									scMarker.crColor = Parser->crColors[ CARR_MARKER_FG_1 + i - 1 ];
									...
									{
										/*
										 *	Compute the line rectangle.
										 */
										GetClientRect( lpcd->hWnd, &rcLine );
										rcLine.top   += cy;
										rcLine.bottom = rcLine.top + Parser->szCharSize.cy;
										rcLine.left   = cx;
										rcLine.right -= nMarkerOffset;
										...
										/*
										 *	Increase the marker offset.
										 */
										nMarkerOffset += Parser->szCharSize.cx;
									}
								}
							}
						}
						...
						if (( lpLine->dwFlags & LNF_BOOKMARK ) == LNF_BOOKMARK )
						{
							...
							/*
							 *	No marker forground color.
							 */
							scMarker.crColor = CLR_DEFAULT;
							...
							if ( Parser->bSelectionMargin )
							{
								...
								RoundRect( hDC, rcClient.top, cy, GetSelMarginWidth( lpcd ) - 1, cy + Parser->szCharSize.cy, Parser->szCharSize.cx, Parser->szCharSize.cy / 2 );
							}
							else
							{
								/*
								 ...
								 */
								rc = rcClient;
								rc.left += GetMarginWidth( lpcd );
								rc.right -= nMarkerOffset;
						...
						{
							/*
							 *	Line numbers larger than will fit on the margin will be converted to
							 *	 ----- for now. Should not be a big problem.
							 */
							int	 nLength;
							TCHAR    sz[ 32 ] = "----------";
							if ( nLine < (( int )pow( 10.0, Parser->nMarginChars )) - 1 ) 
							{
								/*
								 *	Convert the number.
								 */
								_itot_s( nLine + 1, sz, 32, 10 );
								nLength = _tcslen( sz );
							}
							else
								/*
							...
							TextOut( hDC, GetSelMarginWidth( lpcd ) + 3 + ((  Parser->nMarginChars - nLength ) * Parser->szCharSize.cx ), cy, sz, nLength );
						}
						...
						if ( nLine == lpcd->ptCaretPos.y && Parser->bHighlightCurrentLine )
						{
							 ...
							 */
							rcClient.left   = GetMarginWidth( lpcd );
							rcClient.top   += cy;
							rcClient.bottom = rcClient.top + Parser->szCharSize.cy;
							rcClient.right -= nMarkerOffset;
							...
							/*
							 *	No marker foreground color.
							 */
							scMarker.crColor = CLR_DEFAULT;
						}

						/*
						 *	Is this line is completely inside a text selection
						 *	we do not bother to render it here.
						 */
						if ( ! HasMark( lpcd ) || nLine <= lpcd->ptSelStart.y || nLine >= lpcd->ptSelEnd.y )
						{
							/*
							 *	Was there a foreground color defined?
							 */
							if ( scMarker.crColor != CLR_DEFAULT )
							{
								/*
								 *	Yes. Render all text in this color.
								 */
								 scArray = &scMarker;
							}
...
static void PaintControl( HDC hDC, LPCRECT pClipRect, LPCLASSDATA lpcd )
{
	...
	int	nMargin = GetMarginWidth( lpcd );
	...
	if ( Parser->bSelectionMargin )
	{
		 ...
		 */
		rcMargin.right = GetSelMarginWidth( lpcd );
	...
	/*
	 *	Do we have a column marker and do we need to render the
	 *	part right of the column marker in it's own color?
	 */
	if ( Parser->bColumnMarker && Parser->nMarkerOffset && Parser->bColorColumnMarkerOverflow )
	{
		/*
		 *	Render the background left and right of the
		 *	column marker.
		 */
		int xPos = nMargin + (( Parser->nMarkerOffset - lpcd->ptViewPos.x ) * Parser->szCharSize.cx ) - 1;
		RECT rc;
		...
		/*
		 *	Setup top and bottom edges of the rectangle which are
		 *	the same for both the left side and right side rendering.
		 */
		rc.top = rcClient.top;
		rc.bottom = rcClient.bottom;

		/*
		 *	Do we need to render the left side?
		 */
		if ( xPos > rcClient.left )
		{
			rc.left = rcClient.left;
			rc.right = xPos > rcClient.right ? rcClient.right : xPos;
			FillRect( osDC, &rc, ISREADONLY ? lpcd->hbBackgroundRO : lpcd->hbBackground );
		}

		/*
		 *	Do we need to render the right side?
		 */
		if ( xPos < rcClient.right )
		{
			rc.right = rcClient.right;
			rc.left = xPos > rcClient.left ? xPos : rcClient.left;
			FillRect( osDC, &rc, ISREADONLY ? lpcd->hbBackgroundRO : lpcd->hbColumnOverflow );
		}
	}
	else
	{
		/*
		 *	Simply render the background.
		 */
