...
static BOOL DoKeyDown( LPCLASSDATA lpcd, UINT uKeyCode, DWORD dwKeyData )
{
	...
	else if ( uKeyCode == VK_BACK && lpcd->bIncrementalMode )
	{
		/*
		 *	Call the incremental search function with a
		 *	0-character. This will remove the right-most
		 *	character of the search string.
		 */
		IncrementalSearch( lpcd, _T( '\0' ));
		return TRUE;
	}
	else if ( uKeyCode == VK_RETURN && lpcd->bIncrementalMode )
	{
		/*
		 *	Simple clears the incremental mode.
		 */
        ...
        if (( nLen = ToAscii( uKeyCode, ( UINT )dwKeyData, &bStates[ 0 ], ( LPWORD )&bBuf[ 0 ], 0 )) > 0 )
	{
		for ( i = 0; i < nLen; i++ )
		{
			...
			if ( bBuf[ i ] != _T( '\n' ) && bBuf[ i ] != _T( '\r' ))
			{
				...
				{
					/*
					 *	Do the incremental search instead of
					 *	inserting the character.
					 */
					IncrementalSearch( lpcd, bBuf[ i ] );
				}
				else
				{
					/*
...
BOOL SetupHash( POOL pMemPool, LPHASH *lpTable )
{
        ...
        struct
	{
		...
		{ _T("Ctrl Shift p"),		CID_EDIT_PULL,			CTYPE_HARDCODED },
		...
		{ _T("Ctrl Shift s"),		CID_EDIT_SORTLINES,		CTYPE_HARDCODED },
		{ _T("Alt Ctrl c"),		CID_EDIT_COMMENTSELECTION,	CTYPE_HARDCODED },
		{ _T("Alt Ctrl n"),		CID_EDIT_UNCOMMENTSELECTION,	CTYPE_HARDCODED },
		...
		{ _T("Ctrl i"),			CID_FIND_SET_INCREMENTAL_MODE,	CTYPE_HARDCODED },
 ...
 */
void CodeQual2Str( TCHAR cCode, TCHAR cQual, TCHAR *lpszBuffer, int length )
{
	...
	int			i;
        ...
        *lpszBuffer = 0;
		 ...
		 *		NOTE: 128 chars buffer is expected!
         */
        if ( cQual & QUAL_SHIFT ) _tcscat_s( lpszBuffer, length, _T("Shift "));
        if ( cQual & QUAL_CTRL  ) _tcscat_s( lpszBuffer, length, _T("Ctrl "));
        if ( cQual & QUAL_ALT   ) _tcscat_s( lpszBuffer, length, _T("Alt "));
        ...
        for ( i = 0; i < sizeof( Keys ) / sizeof( struct SpecialKey ); i++ )
	{
                if ( Keys[ i ].lpszKey )
		{
                        if (( Keys[ i ].cCode & 0xFF ) == cCode )
			{
                                 ...
                                 *      Append key string.
                                 */
                                _tcscat_s( lpszBuffer, length, Keys[ i ].lpszKey );
                                return;
                        }
                }
        }
	 ...
	 *	Map key code.
        ...
        if ( ToAscii( cCode, MapVirtualKey( cCode, 0 ), ( LPBYTE )&bStates[ 0 ], ( LPWORD )&bBuf[ 0 ], 0 ) > 0 )
                _tcscat_s( lpszBuffer, length, &bBuf[ 0 ] );
}