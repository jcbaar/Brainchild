...
static HFONT CalcPageSize( LPCLASSDATA lpcd, HDC hDC )
{
	...
	if ( Parser->bPrintLineNumbers )
	{
		 ...
		 */
		_itot_s( ArrayGetSize( lpcd->lpLines ), szLN, 25, 10 );
...
static int AppendString( LPTSTR pszBuffer, LPTSTR pszAdd, int nIndex )
{
	 ...
	 *	buffer.
	 */
	_tcsncpy_s( &pszBuffer[ nIndex ],1024 - nIndex,  pszAdd, 1024 - nIndex );
		...
		if ( pszInput[ i ] == _T( '%' ))
		{
			...
			switch ( pszInput[ i ] )
			{
				...
				case	_T( 'p' ):
				{
					 ...
					 */
					_stprintf_s( szNumber, 20, _T( "%ld " ), nPage );
				...
				case	_T( 'P' ):
				{
					 ...
					 *	Convert the number string.
					 */
					_stprintf_s( szNumber, 20, _T( "%ld " ), nTotalPages );
 ...
 *	sized printer font.
 */
static HFONT MakeFont( LPCLASSDATA lpcd, HDC hPrinterDC, BOOL bPrinterFont )
{
	LOGFONT	lf = { 0 };
	HFONT	hNewFont = NULL;
	HDC		hDC;
	...
	{
		/*
		 *	Conversion of the screen font?
		 */
		if ( bPrinterFont == FALSE )
		{
			/*
		...
		{
			/*
			 *	Compute necessary printer font size.
			 */
			lf = Parser->lfPrinterFont;
			lf.lfHeight = MulDiv( lf.lfHeight, GetDeviceCaps( hPrinterDC, LOGPIXELSY ), GetDeviceCaps( hDC, LOGPIXELSY ));
			lf.lfWidth  = MulDiv( lf.lfWidth,  GetDeviceCaps( hPrinterDC, LOGPIXELSX ), GetDeviceCaps( hDC, LOGPIXELSX ));
...
static BOOL ConvertFont( LPCLASSDATA lpcd, HDC hPrinterDC, BOOL bPrinterFont )
{
	/*
	 *	Setup the new font.
	 */
	if (( Parser->hPrinterFont = MakeFont( lpcd, hPrinterDC, bPrinterFont )) != NULL )
		return TRUE;
	return FALSE;
}
...
static void PrintText( LPCLASSDATA lpcd, HDC hDC, BOOL bSelection, int nCopies )
{
	...
	/*
	 *	Use the screen font or printer font for printing?
	 */
	if ( ConvertFont( lpcd, hDC, ! Parser->bPrintScreenFont ) == FALSE )
	{
		/*
		 *	Font creation failed.
		 */
		OkError( lpcd, lpcd->hWnd, GetString( IDS_PRINTFONT_ERROR ));
		return;
	}
	 ...
	 */
	pszFilename = _tcsrchr( lpcd->szFileName, _T( '\\' ));
