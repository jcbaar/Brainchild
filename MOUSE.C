...
BOOL MouseOnSelection( LPCLASSDATA lpcd )
{
	...
	if ( HasMark( lpcd ))
	{
		 ...
		 */
		ptMousePos.x -= GetMarginWidth( lpcd );
...
static BOOL CheckMouseClick( LPCLASSDATA lpcd, int x, int y, DWORD dwButtonState, int nButtonType )
{
	NMMARGINCLICK nmmc;
	int nLineClicked;
	 ...
	 */
	if ( lpcd->bIncrementalMode )
	{
		/*
		 *	Clear the incremental mode.
		 */
		ClearIncrementalMode( lpcd );
	}

	/*
	 *	Was the click inside the margin?
	 */
	if ( GetParent( lpcd->hWnd ) && x < -5 )
	{
		/*
		 *	Adjust the click position to include the
		 *	margin size.
		 */
		x += GetMarginWidth( lpcd );
		
		/*
		 *	Convert mouse position to
		 *	a text position.
		 */
		nLineClicked = lpcd->ptViewPos.y + ( y / Parser->szCharSize.cy );

		/*
		 *	Only send the notification when the click
		 *	was inside the text.
		 */
		if ( nLineClicked >= ArrayGetSize( lpcd->lpLines ))
			return TRUE;

		/*
		 *	Fill in the notification structure.
		 */
		nmmc.hdr.code		= NMBC_MARGINCLICK;
		nmmc.hdr.hwndFrom	= lpcd->hWnd;
		nmmc.hdr.idFrom		= GetWindowLong( lpcd->hWnd, GWL_ID );
		nmmc.nButton		= nButtonType;
		nmmc.nLine		= nLineClicked + 1;
		nmmc.dwButtonState	= dwButtonState;
		nmmc.bDoDefault		= TRUE;

		/*
		 *	Setup the margin in which was clicked.
		 */
		if ( x <= GetSelMarginWidth( lpcd ))
		{
			nmmc.nMargin = MCT_SELECTION;
		}
		else if ( x <= GetLineMarginWidth( lpcd ))
		{
			nmmc.nMargin = MCT_LINENUMBERS;
		}

		/*
		 *	Send the notification.
		 */
		SendMessage( GetParent( lpcd->hWnd ), WM_NOTIFY, nmmc.hdr.idFrom, ( LPARAM )&nmmc );

		/*
		 *	TRUE if we can do the default action.
		 */
		return nmmc.bDoDefault;
	}
	/*
	 *	By default we allow the default action.
	 */
	return TRUE;
}

LRESULT OnRButtonDown( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	int	x = ( int )( short )( GET_X_LPARAM( lParam ) - GetMarginWidth( lpcd ));
	int	y = ( int )( short )( GET_Y_LPARAM( lParam ));

	/*
	 *	Send the marginclick notification.
	 */
	CheckMouseClick( lpcd, x, y, wParam, MK_RBUTTON );
	return 0;
}

LRESULT OnMButtonDown( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	int	x = ( int )( short )( GET_X_LPARAM( lParam ) - GetMarginWidth( lpcd ));
	int	y = ( int )( short )( GET_Y_LPARAM( lParam ));
	 ...
	 *	Send the marginclick notification.
	 */
	CheckMouseClick( lpcd, x, y, wParam, MK_MBUTTON );
	return 0;
}
...
LRESULT OnLButtonDown( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	...
	/*
	 *	Can we perform the default action following
	 *	this click?
	 */
	if ( CheckMouseClick( lpcd, x, y, wParam, MK_LBUTTON ) == FALSE )
		return 0;
...
static void DoTooltip( LPCLASSDATA lpcd, BOOL bShow )
{
	...
	if ( pszLink )
	{
		...
		if ( _tcslen( pszFormat ) + _tcslen( pszLink ) < 1024 )
		{
			 ...
			 *	Format the output.
			 */
			_stprintf_s( szBuffer, 1024, pszFormat, pszLink );
...
LRESULT OnMouseMove( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	...
	if ( lpcd->nDragTimerID != 0 )
	{
		...
		/*
		 *	If one of the previous selection points was outside the current
		 *	view we re-render the lines between the current selection
		 *	point and the start or end of the current view.
		 */
		if ( nOldEnd > lpcd->ptSelEnd.y && nOldEnd > lpcd->ptViewPos.y + lpcd->szViewSize.cy )
		{
			RenderLines( lpcd, lpcd->ptSelEnd.y + 1, lpcd->ptViewPos.y + lpcd->szViewSize.cy );
		}
		if ( nOldStart < lpcd->ptSelStart.y && nOldStart < lpcd->ptViewPos.y )
		{
			RenderLines( lpcd, lpcd->ptViewPos.y, lpcd->ptSelStart.y - 1 );
		}
...
LRESULT OnTimer( HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	...
	if ( wParam == IDT_DRAGTIMER )
	{
		 ...
		 *	client area?
		 */
		if (      ptMouse.x < GetMarginWidth( lpcd )) ptView.x--;
...
LRESULT OnMouseWheel(  HWND hWnd, WPARAM wParam, LPARAM lParam, LPCLASSDATA lpcd )
{
	...
	if ( abs( lpcd->nWheelDelta ) >= 120 )
	{
		 ...
		 */
		int nLines = ( lpcd->nWheelDelta / WHEEL_DELTA ) * Parser->nScrollLines;
